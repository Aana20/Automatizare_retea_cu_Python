...
import requests
    ...
    # return cookeis ca sa pot sa-l folosesc in  main ca altfel da null la cokies
def login_eve():
    ...
    if login.status_code==200:
        ...
        return None
#pentru verificare daca nodul e oprit
def node_status_check(cookies,node_name,lab_path="/Licenta/Automatizare.unl",eve_ip="192.168.238.128"):
    ...
    for node_id, node in nodes.items():
        if node["name"].strip().upper() == node_name.strip().upper():
            return node["status"] == 2  # 2 = running
